...
class COLMAPMVSPipeline:
    ...
    def process_session(
        ...
        # 後処理1: 原点からの距離でフィルタリング
        # COLMAP座標系では原点が初期カメラ位置付近にあるため、
        # 原点からの距離でフィルタすることで測定範囲を制限
        colmap_config = self.config.get('colmap', {})
        distance_filter = colmap_config.get('distance_filter', {})
        if distance_filter.get('enable', False):
            ...
            if progress_callback:
                progress_callback(85, f"Filtering points: {min_distance}m - {max_distance}m from origin...")
            ...
            print(f"  Distance filter (origin, {min_distance}-{max_distance}m): {len(pcd.points):,} → {len(pcd_filtered.points):,} points")
            pcd = pcd_filtered
