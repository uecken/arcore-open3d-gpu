...
@app.get("/scenes/{job_id}/mesh_aligned.ply")
async def get_mesh_aligned(job_id: str):
    """局所補正済みメッシュPLYファイル"""
    path = RESULTS_DIR / job_id / "mesh_aligned.ply"
    if not path.exists():
...
async def get_trajectory_colmap_corrected(job_id: str):
    ...
    if not path.exists():
        raise HTTPException(404, "Corrected COLMAP trajectory not found")
