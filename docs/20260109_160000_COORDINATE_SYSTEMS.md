...
Procrustes分析による自動変換は**カメラ位置**を合わせますが、**重力方向（垂直方向）**は考慮しません。
そのため、生成されたメッシュは傾いて表示される場合があります。
...
| 要因 | 影響 |
|------|------|
| セッション開始時のデバイス姿勢 | ARCoreの「上」方向が決まる |
| COLMAPの座標系構築 | 任意の向きになる |
| Procrustes回転 | 位置は合うが、傾きが残る |
...
1. **ARCore重力ベクトルの活用**: セッション中の平均重力方向を記録
2. **自動重力補正**: ARCoreの「上」方向とメッシュの「上」方向を自動整合
3. **セッション毎の補正値保存**: 一度調整した値を記録
