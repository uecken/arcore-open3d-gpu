...
## 追記: メッシュ傾き補正（2026-01-09 20:15追加）

### ⚠️ 自動変換の限界

上記すべての方式（方式2、方式3、方式3改良）において、
Procrustes分析による自動変換後も、メッシュが**傾いて表示される**場合があります。
...
ARCore、COLMAP、Viewerはそれぞれ異なる座標系を使用しており、
変換後の傾きがどの程度になるかはセッション毎に異なります。
...
**注意: X/Y/Z軸はViewer（Three.js）の座標系**
- ARCoreやCOLMAP座標系のどの軸に対応するかは不明
